...
* For recent events, go to `Aiven Console <https://console.aiven.io/>`_ > your service's page > **Logs** view (available from the sidebar)

  .. Tip::

     Logs can be browsed back in time, but scrolling up several thousand lines is not very convenient.

* For downloading logs programmatically with the :ref:`dedicated command <avn-service-logs>`, use :doc:`Aiven CLI </docs/tools/cli>` supports
