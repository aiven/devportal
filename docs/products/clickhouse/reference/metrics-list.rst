...
The metrics/dashboard integration in the Aiven console enables you to push ClickHouse® metrics to an external endpoint like Datadog or to create an integration and a prebuilt dashboard in Aiven for Grafana®. For more information on enabling the integration, see :doc:`../howto/report-metrics-grafana`. This article describes the default dashboard created in Grafana for any ClickHouse instance.
...
1. The ClickHouse dashboards show all tables and indexes for all logical databases since Aiven cannot determine tables or indexes relevance.
...
ClickHouse metrics prebuilt dashboard
-------------------------------------

The ClickHouse default dashboard is split into several sections under two main categories: Generic and ClickHouse. **Generic** metrics are not specific to the type of service running on the node and mostly related to CPU, memory, disk, and network. **ClickHouse** metrics are specific for the service.
...
.. image:: /images/products/ClickHouse/metrics-dashboard-overview.png
    :alt: Grafana Dashboard for ClickHouse Overview Section
...
.. image:: /images/products/ClickHouse/metrics-dashboard-system.png
    :alt: Grafana Dashboard for ClickHouse System Metrics Section
...
For most metrics, the metric name identifies the internal ClickHouse statistics view. See the `ClickHouse documentation <https://www.ClickHouse.org/docs/current/static/monitoring-stats.html>`_ for more detailed explanations of the various metric values.

Metrics that are currently recorded but not shown in the default dashboard include ``ClickHouse.pg_stat_bgwriter`` and ``ClickHouse.pg_class`` metrics as a whole, as well as some individual values from other metrics.

ClickHouse overview
"""""""""""""""""""

The metrics in the ClickHouse overview section are grouped by logical database. In addition, some metrics are grouped by host.


.. image:: /images/products/ClickHouse/metrics-dashboard-pg-overview.png
    :alt: Grafana Dashboard for ClickHouse database Overview Section
...
.. list-table::
    ...
    * - ``Connections``
      ...
      - Each connection puts a large burden on the ClickHouse server and this number :doc:`should typically be fairly small even for large plans <pg-connection-limits>`. Use connection pooling to :doc:`reduce the number of connections <../concepts/pg-connection-pooling>` to the actual database server.
    ...
    * - ``Disk block reads / sec``
      - The number of 8 kB disk blocks that ClickHouse reads per second, excluding reads that were satisfied by the buffer cache.
      - The read operations may have been satisfied by the operating system's file system cache.
    * - ``Buffer cache disk block reads / sec``
      - The number of 8 kB disk blocks that ClickHouse reads per second that were already in buffer cache.
      -
    * - ``Temp files created / min``
      - The number of temporary files that ClickHouse created per minute.
...
.. image:: /images/products/ClickHouse/metrics-dashboard-pg-indexes.png
    :alt: Grafana Dashboard for ClickHouse database Indexes Section
...
.. image:: /images/products/ClickHouse/metrics-dashboard-pg-tables.png
    :alt: Grafana Dashboard for ClickHouse database Indexes Section
...
.. list-table::
    ...
    * - Parameter Name
      ...
      - Additional Notes
    * - ``Table size``
      - The size of tables, excluding indexes and `TOAST data <https://www.ClickHouse.org/docs/current/storage-toast.html>`_
      -
    * - ``Table size total``
      - The total size of tables, including indexes and `TOAST data <https://www.ClickHouse.org/docs/current/storage-toast.html>`_
      -
    * - ``Table seq scans / sec``
...
.. image:: /images/products/ClickHouse/metrics-dashboard-pg-vacuum.png
    :alt: Grafana Dashboard for ClickHouse database Vacuum and Analyse Section
...
ClickHouse miscellaneous
""""""""""""""""""""""""

This section contains ClickHouse metrics graphs that are not covered by the previous sections.


.. image:: /images/products/ClickHouse/metrics-dashboard-pg-miscellaneous.png
    :alt: Grafana Dashboard for ClickHouse database Miscellaneous Section
