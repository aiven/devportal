...
.. Danger::
    Several features of PostgreSQL are known to be **broken** by the default transaction-based pooling, and so these features **must not be used by the application when in this mode** - these features are described in the `official PgBouncer features page <https://www.pgbouncer.org/features>`_.
