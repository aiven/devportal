...
If you want to enable any writes to the database for a limited amount of time, send the following ``POST`` request using :doc:`Aiven APIs </docs/tools/api/index>` and replacing the ``PROJECT_NAME`` and ``SERVICE_NAME`` placeholders:
