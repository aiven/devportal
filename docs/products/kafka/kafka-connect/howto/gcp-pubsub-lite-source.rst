...
* ``GCP_SERVICE_KEY``: A valid GCP service account key for the ``GCP_PROJECT_NAME``. To create the project key review the :ref:`dedicated document <gcp-bigquery-sink-connector-google-account>`
  ...
  .. Warning::
     ...
     The GCP Pub/Sub Lite source connector accepts the ``GCP_SERVICE_KEY`` JSON service key as a string, therefore all  ``"`` symbols within it must be escaped ``\"``.
...
5. Paste the connector configuration (stored in the ``pubsub_lite_source.json`` file) in the form
6. Click on **Apply**
