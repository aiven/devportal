...
Data eviction policy is one of the most important Redis settings in this case, it is available in the Aiven web console. 
...
Aiven does not have a rate limit traffic for Redis services because limiting only the relevant write operations requires specialized Redis proxy, and restricting all traffic can affect non-write traffic. 
